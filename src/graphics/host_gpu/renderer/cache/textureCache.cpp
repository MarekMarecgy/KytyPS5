...
void TextureCache::MaterializeDccClear(ImageId id, const ImageDesc& desc,
	...
	// Draws to a bound DCC target keep re-dirtying its metadata, and each readback submits the
	// open recording and waits for the GPU. Resolve the same layers once per recording unless the
	// guest CPU rewrote the metadata since.
	if (!m_buffer_cache.IsRegionCpuModified(range.address, range.size)) {
		std::scoped_lock           lock {m_lock};
		const Image::DccClearCheck check {m_scheduler.CurrentTick(), first, count, image_first};
		if (m_slot_images[id].dcc_clear_checked == check) {
			return;
		}
	}
	...
	{
		std::scoped_lock lock {m_lock};
		m_slot_images[id].dcc_clear_checked = {m_scheduler.CurrentTick(), first, count,
		                                       image_first};
	}
