...
struct PipelineCache::ProgramCache {
	...
	ShaderProgram Get(const ShaderParams& params, InputInfo& input_info,
		...
		// KYTY_SKIP_RAY_TRACING=1 skips ray-tracing dispatches instead of emulating them,
		// for isolating ray-tracing regressions.
		static const bool skip_ray_tracing = [] {
			const char* value = std::getenv("KYTY_SKIP_RAY_TRACING");
			return value != nullptr && value[0] == '1';
		}();
		options.emulate_ray_tracing = !skip_ray_tracing;
		...
		if (translated.skip_dispatch) {
			DumpShaderOriginal("cs_skipped", params.hash, params.code, translated.decoded_dump);
