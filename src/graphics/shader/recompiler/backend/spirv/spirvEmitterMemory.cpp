...
void EmitReadConstBuffer(ValueEmitContext& ctx, const IR::Inst& inst) {
	...
	auto& state = ctx.state;
	if (mem.kind == IR::ResourceKind::IndirectBuffer) {
		// V# selected at run time: read through the guest page table. S_BUFFER_LOAD ignores
		// the low two offset bits and checks the dword against NUM_RECORDS (bytes, or
		// elements when a stride is set).
		const auto& handle = *inst.Arg(0).ResolveInstruction();
		const auto  word1  = ctx.Arg(handle, 1);
		const auto  field  = [&](uint32_t word, uint32_t first, uint32_t count) {
			return EmitBitFieldUExtract(state, word, ConstantU32(state, first),
		...
		const auto size =
		    Select(state, TypeU32(state),
		           ...
		           records, Binary(state, spv::OpIMul, TypeU32(state), stride, records));
		const auto byte      = Binary(state, spv::OpBitwiseAnd, TypeU32(state),
		                              Binary(state, spv::OpIAdd, TypeU32(state), ctx.Arg(inst, 1),
		                                     ConstantU32(state, mem.offset)),
		                              ConstantU32(state, ~3u));
		const auto in_bounds = AndCondition(
		    state, Binary(state, spv::OpULessThan, TypeBool(state), byte, size),
		    Binary(state, spv::OpUGreaterThanEqual, TypeBool(state),
		           Binary(state, spv::OpISub, TypeU32(state), size, byte), ConstantU32(state, 4)));
		const auto base  = DeviceAddressFromWords(state, ctx.Arg(handle, 0), field(word1, 0, 16));
		const auto guest = Binary(state, spv::OpIAdd, TypeScalarU64(state), base,
		                          Unary(state, spv::OpUConvert, TypeScalarU64(state), byte));
		ctx.Define(inst, LoadBda(ctx, guest, in_bounds, 32u));
		return;
	}
