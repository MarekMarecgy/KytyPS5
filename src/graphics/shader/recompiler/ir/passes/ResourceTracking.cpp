	...
	void Collect(Inst& inst) {
		...
		if (buffer != BufferAccess::None) {
			if (!GetHandle(inst.Arg(0), ValueOpcode::GetBufferResource, 4, flags.pc, handle,
			               source)) {
				const bool indirect_kind = memory.kind == ResourceKind::Buffer ||
				                           (memory.kind == ResourceKind::ScalarBuffer &&
				                            op == ValueOpcode::ReadConstBuffer);
				if (!indirect_kind || !memory.SupportsIndirectBufferLoad(op)) {
					Fail(flags.pc,
					     "buffer descriptor is not a valid runtime value; GPU-selected access "
					     "requires a raw DWORD x2/x3/x4 or scalar DWORD load");
				}
