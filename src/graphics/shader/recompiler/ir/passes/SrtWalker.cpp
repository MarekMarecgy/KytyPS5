...
constexpr uint64_t NullPageSize = 0x10000u;
...
private:
	...
	void Collect(Value value, uint32_t use_pc) {
		...
		// A read whose address changes between loop iterations (e.g. BVH traversal loading
		// instance nodes) has no single value to precompute; keep it as a runtime read.
		if (!offset.IsImmediate() || offset.GetType() != Type::U32 ||
		    AddressDependsOnVariantPhi(*inst)) {
	...
	bool AddressDependsOnVariantPhi(const Inst& read) const {
		std::vector<Value>              pending;
		std::unordered_set<const Inst*> visited;
		const auto*                     handle = read.Arg(0).Resolve().TryInstruction();
		if (handle != nullptr) {
			for (size_t index = 0; index < handle->NumArgs(); index++) {
				pending.push_back(handle->Arg(index));
			}
		}
		pending.push_back(read.Arg(1));
		while (!pending.empty()) {
			...
			if (inst->GetOpcode() == ValueOpcode::Phi &&
			    ResolveInvariantPhi(m_program, current).IsEmpty()) {
				return true;
			}
			for (size_t index = 0; index < inst->NumArgs(); index++) {
...
bool SrtWalker::EvaluateRawRead(const Inst& inst, uint64_t& result) {
	...
	if (m_runtime.read_memory != nullptr) {
		...
		// SRT planning collects reads regardless of control flow, so a read guarded by a
		// branch that is not taken can fold to a null base. Dereferencing it would crash the
		// host; treat the null page as zero-filled instead.
		if (address < NullPageSize) {
			result = 0;
			return true;
		}
...
bool SrtWalker::RefreshFlatBuffer(std::vector<uint32_t>& flat) {
	...
	for (const auto& read: m_program.srt_reads) {
		...
		const auto describe = [&](const char* why) {
			const auto*       inst = read.value.Resolve().TryInstruction();
			const std::string name = inst != nullptr
			                             ? std::string(ValueOpcodeName(inst->GetOpcode()))
			                             : std::string("-");
			std::printf(
			    "SRT flat read failed (%s): hash=0x%016llx slot=%u clean=%d op=%s pc=0x%08x\n", why,
			    static_cast<unsigned long long>(m_program.shader_hash), read.flat_offset,
			    clean ? 1 : 0, name.c_str(),
			    inst != nullptr && IsRawRead(m_program, *inst) ? inst->Flags<MemoryFlags>().pc
			                                                   : 0u);
			return false;
		};
		if (clean &&
		    (m_clean_evaluator == nullptr || m_runtime.read_specialization_memory == nullptr)) {
			return describe("clean slot without specialization memory");
		}
