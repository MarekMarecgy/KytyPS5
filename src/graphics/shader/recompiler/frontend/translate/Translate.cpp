...
void IncludeInstructionVectorRegisters(const Decoder::Instruction& inst, uint32_t& vector_limit) {
	...
	if ((inst.opcode == Decoder::Opcode::IMAGE_BVH_INTERSECT_RAY ||
	     inst.opcode == Decoder::Opcode::IMAGE_BVH64_INTERSECT_RAY) &&
	    inst.image_nsa_dwords == 0u) {
		include_vector(inst.src0, inst.image_address_components);
	}
		...
		if (options.stage == ShaderType::Compute) {
			...
			const auto primitives = entry_ir.Select(
			    ...
			    entry_ir.IAdd(IR::U32(entry_ir.Emit(IR::ValueOpcode::UDiv32,
			                  ...
			                  u32(1)));
			const auto wave =
			    entry_ir.ShiftRightLogical(local, u32(options.wave_size == 32u ? 5u : 6u));
			...
			const auto wave_info = entry_ir.BitwiseOr(
			    ...
			    u32(((total_threads + options.wave_size - 1u) / options.wave_size) << 28u));
			entry_ir.SetScalarReg(
